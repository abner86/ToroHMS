(function (a, b) {
    ...
    function Wc(a, b, c) {
        ...
        }, j = h.promise({elem: a, props: p.extend({}, b), opts: p.extend(true, {specialEasing: {}}, c), originalProperties: b, originalOptions: c, startTime: Nc || Uc(), duration: c.duration, tweens: [], createTween: function (b, c, d) {
    ...
    p.extend({cache: {}, deletedIds: [], uuid: 0, expando: "JFBase" + (p.fn.jfbase + Math.random()).replace(/\D/g, ""), noData: {embed: true, object: "clsid:D27CDB6E-AE6D-11cf-96B8-444553540000", applet: true}, hasData: function (a) {
    ...
    if (!p.support.style) {
        p.attrHooks.style = {get: function (a) {
    ...
    p.event = {add: function (a, c, d, e, f) {
        ...
        for (j = 0; j < c.length; j++) {
            ...
            r = p.event.special[l] || {};
            n = p.extend({type: l, origType: k[1], data: e, handler: d, guid: d.guid, selector: f, namespace: m.join(".")}, o);
            q = i[l];
            if (!q) {
                q = i[l] = [];
                q.delegateCount = 0;
    ...
    }, customEvent: {getData: true, setData: true, changeData: true}, trigger: function (c, d, f, g) {
    ...
    }, special: {load: {noBubble: true}, focus: {delegateType: "focusin"}, blur: {delegateType: "focusout"}, beforeunload: {setup: function (a, b, c) {
        if (p.isWindow(this)) {
    ...
    (function (a, b) {
        ...
        }, radio: _("radio"), checkbox: _("checkbox"), file: _("file"), password: _("password"), image: _("image"), submit: ab("submit"), reset: ab("reset"), button: function (a) {
            var b = a.nodeName.toLowerCase();
    ...
    var cb = /Until$/, db = /^(?:parents|prev(?:Until|All))/, eb = /^.[^:#\[\.,]*$/, fb = p.expr.match.needsContext, gb = {children: true, contents: true, next: true, prev: true};
        ...
        if (this[0]) {
            ...
            if (k.length) {
                p.each(k, function (a, b) {
                    if (b.src) {
                        if (p.ajax) {
                            p.ajax({url: b.src, type: "GET", dataType: "script", async: false, global: false, "throws": true})
                        } else {
                            p.error("no ajax")
                        }
                    } else {
    ...
    var Hb, Ib, Jb, Kb = /alpha\([^)]*\)/i, Lb = /opacity=([^)]*)/, Mb = /^(top|right|bottom|left)$/, Nb = /^(none|table(?!-c[ea]).+)/, Ob = /^margin/, Pb = new RegExp("^(" + q + ")(.*)$", "i"), Qb = new RegExp("^(" + q + ")(?!px)[a-z%]+$", "i"), Rb = new RegExp("^([-+])=(" + q + ")", "i"), Sb = {}, Tb = {position: "absolute", visibility: "hidden", display: "block"}, Ub = {letterSpacing: 0, fontWeight: 400}, Vb = ["Top", "Right", "Bottom", "Left"], Wb = ["Webkit", "O", "Moz", "ms"], Xb = p.fn.toggle;
    ...
    }}}, cssNumber: {fillOpacity: true, fontWeight: true, lineHeight: true, opacity: true, orphans: true, widows: true, zIndex: true, zoom: true}, cssProps: {"float": p.support.cssFloat ? "cssFloat" : "styleFloat"}, style: function (a, c, d, e) {
    ...
    p.fn.load = function (a, c, d) {
        ...
        p.ajax({url: a, type: f, dataType: "html", data: c, complete: function (a, b) {
            if (d) {
    ...
    var Nc, Oc, Pc = /^(?:toggle|show|hide)$/, Qc = new RegExp("^(?:([-+])=|)(" + q + ")([a-z%]*)$", "i"), Rc = /queueHooks$/, Sc = [Yc], Tc = {"*": [function (a, b) {
    ...
    p.Animation = p.extend(Wc, {tweener: function (a, b) {
        if (p.isFunction(a)) {
            b = a;
            a = ["*"]
        } else {
    ...
    p.Tween = Zc;
    Zc.prototype = {constructor: Zc, init: function (a, b, c, d, e, f) {
        this.elem = a;
        this.prop = c;
        this.easing = e || "swing";
        this.options = b;
    ...
    Zc.propHooks = {_default: {get: function (a) {
        var b;
    ...
    Zc.propHooks.scrollTop = Zc.propHooks.scrollLeft = {set: function (a) {
...
JFBase.expr[':'].regex = function (elem, index, match) {
    var be = match[3].split(','), aO = /^(data|css):/, attr = {method: be[0].match(aO) ? be[0].split(':')[0] : 'attr', property: be.shift().replace(aO, '')}, regexFlags = 'ig', regex = new RegExp(be.join('').replace(/^\s+|\s+$/g, ''), regexFlags);
...
function obj() {
    ...
    this.bj = {ak: 0, bS: "", bD: 8, bz: function (s) {
...
JC.debug = function (string) {
    ...
    if (!d.length) {
        ...
        d.css({position: 'absolute', left: 0, top: 0, background: '#000000', color: '#ffffff', zIndex: 100000}).attr('id', 'JCDebugBar');
    }
    d.html(string);
    d.show();
};
