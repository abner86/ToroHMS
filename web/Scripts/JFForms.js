(function (a) {
    ...
    a.fn.JFforms.JFselectBox = function () {
        ...
        a(".custom-select").each(function (b) {
            var c = a(this);
            var d = c.attr("id");
            var e = a("#" + d);
            var f = '<div class="custom-dropdown" id="' + d + 'JFElement">' + '<div class="custom-head">' + '<span href="#" class="current">' + a(c).children("option:selected").text() + "</span>" + '<span class="selector"><a></a></span>' + "</div>" + '<div class="custom-body"><ul>';
            var g = new Array;
            ...
            h.on("click", function () {
                ...
                var c = a("option", "#" + d);
                a("option:eq(" + b + ")", e).attr("selected", "selected");
                a(this).addClass("selected");
                a(this).siblings().removeClass("selected");
                e.change();
                a("#" + d + "JFElement").find(".custom-head .current").text(a(this).text());
                a(this).parent().find("li").slideToggle()
            })
        });
        a(".custom-dropdown .custom-head").on("click", function (b) {
            var c = a(this).parents("div.custom-dropdown").find(".custom-body");
            var d = a(this).parent().attr("id");
            if (c.is(":visible")) {
                a(document).find("div.custom-dropdown:not(#" + d + ")").find(".custom-body ul li").slideUp();
                a(document).find("div.custom-dropdown:not(#" + d + ")").find(".custom-body").css("z-index", "2");
                c.css("z-index", "9999")
            }
            a(this).parents("div.custom-dropdown:eq(0)").find(".custom-body ul li").slideToggle("1000", function () {
                if (c.height() === 0) {
                    c.css("z-index", "2")
                }
            })
    ...
    a.fn.JFforms.JFcheckRadio = function () {
        ...
        b.each(function (b) {
            if (a(this).is("[type=checkbox],[type=radio]")) {
                ...
                a('<div class="custom-' + e + '"></div>').insertBefore(c).append(c, d);
                ...
                c.bind("updateState", function () {
                    if (c.is(":checked")) {
                        if (c.is(":radio")) {
                            f.each(function () {
                                a("label[for=" + a(this).attr("id") + "]").removeClass("checked")
                            })
                        }
                        d.addClass("checked")
                    } else {
                        d.removeClass("checked checkedHover checkedFocus")
                    }
    ...
    a.fn.JFforms.fixWidths = function () {
        var b = JC.GRID.isScrollbarVisible() ? a(window).width() + JC.GRID.scrollbarWidth() : a(window).width();
        a("form input:not([type=button]), form textarea, form fieldset").each(function () {
            var c = a(this);
